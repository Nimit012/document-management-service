...
var GoogleAuthHelper = class {
	...
	*/
	createAuthClient(impersonateEmail) {
		try {
			return new googleapis.google.auth.GoogleAuth({
	...
	*/
	async createDriveClient(impersonateEmail) {
		...
		try {
			...
			const driveClient = googleapis.google.drive({
				version: "v3",
				auth
			});
...
exports.DocumentStorageError = DocumentStorageError;
